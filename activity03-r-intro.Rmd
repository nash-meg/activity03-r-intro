...
**Response**: The error "number of rows of result is not a multiple of vector length (arg 3)" means that the third argument (punctuation) is not the same length as the other vectors. To solve this, I added a / and then repeated the punctuation in the vector to make it the same length.
...
**Response**: The first error was a typo in data.frame (missing the .). The second arrow was incorrectly creating an object so I flipped the sides and used a <-.
